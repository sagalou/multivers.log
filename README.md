...
- [`AGENTS.md`](./AGENTS.md) — prompts système, outils, boucle de l'agent
- [`JOURNAL.md`](./JOURNAL.md) — ce que l'IA a produit de faux et ce que nous avons corrigé
...
```
[ Navigateur ]                          front React + Vite, port 5173
      |
      | HTTP  (le front ne parle qu'a notre API, jamais au fournisseur du modele)
      v
[ FastAPI ]                             back Python, port 8000
      |
      +--> /upload      extraction PDF / CSV / notes / OCR  --> chunks --> SQLite
      +--> /documents   liste et statut des documents
      +--> /chunks/{id} passage complet, pour ouvrir une citation
      +--> /ask         boucle agent
      |                   1. le modele choisit un outil
      |                   2. on l'execute et on lui rend le resultat
      |                   3. il recommence, 5 tours au maximum
      |                   4. il redige, les citations sont verifiees
      +--> /report      synthese du corpus
      +--> /tools       activer ou desactiver un outil a chaud
      |
      v
[ SQLite ]  documents, chunks, chunks_fts (index plein texte FTS5)
      |
      v
[ NVIDIA NIM ]  API compatible OpenAI, appelee uniquement par le back
```

Détail des couches et diagramme mermaid dans [`ARCHITECTURE.md`](./ARCHITECTURE.md).
Contrat front/back figé dans [`API_CONTRACT.md`](./API_CONTRACT.md).
Répartition palier par palier dans [`REPARTITION.md`](./REPARTITION.md).
...
- **Les embeddings et la recherche sémantique.** Ils auraient réglé les fautes de frappe et les reformulations. Écartés en V1 : ils demandent un appel d'indexation supplémentaire à chaque document et une dépendance de plus, pour un gain incertain sur un corpus de vocabulaire technique répétitif. La limite est assumée et documentée plus bas.
- **Docker.** Une seule commande pour tout lancer aurait été confortable. Écarté : deux services suffisent, et l'image aurait allongé le temps d'installation chez quelqu'un qui clone le projet, ce qui est précisément ce qu'on cherchait à raccourcir.
- **Un modèle plus grand.** `llama-3.1-70b` tient mieux les consignes, mais met 18 secondes par question contre 3. Nous avons gardé le 8b et assumé son instabilité, mesurée : le score d'évaluation oscille entre 3 et 5 sur 5 selon les exécutions.
- **Changer de fournisseur une seconde fois.** Après la migration de Gemini vers NVIDIA, les latences se sont dégradées. Nous avons préféré changer de modèle, une ligne de configuration, plutôt que de refaire une migration en fin de parcours.
...
| 5 · Durcissement | validé — délais maximum, limites de taille, jeu d'évaluation 5 cas, tests |
| 6 · Livraison | AGENTS.md, JOURNAL.md, README final, tag v1.0 |
